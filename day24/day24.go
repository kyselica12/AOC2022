...
func main() {
	...
	for len(queue) > 0 {
		...
		for _, move := range moves {
			if move.x < 0 || move.y < 0 || move.x >= height || move.y >= width || storms_pos[move] || walls[move] || visited[move] {
				continue
			}
			visited[move] = true
			queue = append(queue, State{move, s.t + 1})
		}
	}
}
